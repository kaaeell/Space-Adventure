...
ACHIEVEMENTS = {"first_mission":"First mission!","explorer":"Traveled 2000+ km!","fuel_finder":"Found fuel in nebula!","millionaire":"Earned 10,000 credits!","legend":"50 missions!","streak":"5 in a row!","bounty_hunter":"Defeated a bounty!","researcher":"All research!","pet_finder":"Found a pet!","lucky":"Lucky day!","traveler":"10000 km total!","pirate_slayer":"10 pirates!","nebula_expert":"5 nebulae!","comedian":"10 jokes!","collector":"10 items!","ship_namer":"Named your ship!","planet_lover":"All planets!","fuel_horder":"5000 fuel!","miner":"50 asteroids!","alien_friend":"10 aliens!","quest_master":"10 quests!","pizza_lover":"Ate 10 pizzas!","star_gazer":"Observed 50 stars!"}
PETS = ["Space Dog","Robot Cat","Alien Hamster","Tiny Dragon","Quantum Fox","Space Penguin","Star Octopus","Nebula Unicorn"]
JOKES = ["Why did the star go to school? To get brighter!","What do astronauts use for pants? An asteroid belt!","How do you organize a space party? You planet!","What's an astronaut's favorite key? The space bar!","Why did the alien cross the galaxy? To get to the other side!"]
NEBULAE = {"Orion":(1340,-220),"Eagle":(7000,0),"Helix":(695,280),"Crab":(6500,190),"Skull":(4200,-500)}
SHOP = {"Dark Crystal":500,"Warp Core":2000,"Quantum Shield":1500,"Space Pizza":50,"Anomaly Scanner":800,"Research Data":400,"Telescope":300}
SHIP_NAMES = ["Star Explorer","Cosmic Wanderer","Nebula Rider","Void Seeker","Galaxy Hopper","Starlight","Dark Star"]
SPACE_FACTS = ["A day on Venus is longer than a year.","Saturn's rings are made of ice and rock.","Jupiter is the largest planet.","Space is completely silent.","There are more stars than grains of sand.","The sun is actually white, not yellow."]
SPACE_WEATHER = ["Solar winds are calm ☀️","Cosmic radiation is normal","A solar flare just passed!","Perfect conditions for travel!","Auroras visible today!","Magnetic field is stable"]
...
def check_quest():
    ...
    if quest_progress >= quest["goal"]:
        ...
        if p["quests_completed"] >= 10: unlock_ach("quest_master")
        gen_quest()
...
def tell_joke():
    ...
    p["jokes_told"] += 1
    if p["jokes_told"] >= 10: unlock_ach("comedian")
...
def alien_encounter():
    ...
    p["aliens_met"] += 1
    if p["aliens_met"] >= 10: unlock_ach("alien_friend")
...
def observe_stars():
    ...
    if p["stars_observed"] >= 50: unlock_ach("star_gazer")
    if quest["type"] == "stars":
...
def view_inventory():
    ...
    if p["inventory"]:
        print("\nYour items:")
        for i, item in enumerate(p["inventory"], 1): print(f"{i}. {item}")
...
def eat_pizza():
    ...
    if "Space Pizza" in p["inventory"]:
        ...
        if p["space_pizza_eaten"] >= 10: unlock_ach("pizza_lover")
        if quest["type"] == "pizza":
            quest_progress += 1
            check_quest()
...
def mission():
    ...
    if p["fuel"] < fuel_needed:
        ...
        if choice == "3":
            ...
            if random.random() < 0.6 + (p["luck"] * 0.02):
                ...
                if p["total_fuel_collected"] >= 5000: unlock_ach("fuel_horder")
                if p["asteroids_mined"] >= 50: unlock_ach("miner")
            else:
    ...
    if p["missions"] == 1: unlock_ach("first_mission")
    if p["credits"] >= 10000: unlock_ach("millionaire")
    if p["missions"] >= 50: unlock_ach("legend")
    if p["streak"] >= 5: unlock_ach("streak")
    if p["record"] >= 2000: unlock_ach("explorer")
    if p["total_distance"] >= 10000: unlock_ach("traveler")
    if len(p["inventory"]) >= 10: unlock_ach("collector")

    if start_name != "Unknown" and start_name not in p["visited_planets"]: p["visited_planets"].append(start_name)
    if end_name != "Unknown" and end_name not in p["visited_planets"]: p["visited_planets"].append(end_name)
    
    all_planets = [name for name, _ in PLANETS.values()]
    if len(set(p["visited_planets"]) & set(all_planets)) >= len(all_planets): unlock_ach("planet_lover")
...
def bounty():
    ...
    if my_hp > 0:
        ...
        if p["pirates_killed"] >= 10: unlock_ach("pirate_slayer")
        crew_xp(30)
    else:
...
def nebula():
    ...
    if choice.isdigit() and 1 <= int(choice) <= len(NEBULAE):
        ...
        time.sleep(1)
        p["nebulae_visited"] += 1
        if p["nebulae_visited"] >= 5: unlock_ach("nebula_expert")
        roll = random.random()
...
def random_fun():
    header("🎲 RANDOM FUN")
    action = random.choice(["joke","pet","luck","treasure","dance","fact","weather","alien","pizza","stargaze"])
...
def stats():
    ...
    print(f"✨ Stars Observed: {p.get('stars_observed', 0)}")

    if p["achievements"]:
        print("\n🏅 Achievements:")
        for a in p["achievements"]: print(f"  • {ACHIEVEMENTS[a]}")
    if p["pets"]:
        print("\n🐾 Pets:")
        for pet in p["pets"]: print(f"  • {pet}")
    if p["inventory"]:
        print("\n📦 Inventory:")
        for item in p["inventory"]: print(f"  • {item}")
